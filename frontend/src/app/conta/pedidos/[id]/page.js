...
export default function OrderDetailPage() {
  ...
  return <main className={styles.page}><div className={styles.toolbar}><h1>Pedido #{order.id}</h1><Link href="/conta/pedidos">Voltar</Link></div><section className={styles.panel}><p>Status: <strong>{order.status}</strong></p><ul>{(order.items || []).map((item, index) => <li key={`${item.product_id}-${index}`}><strong>{item.quantity}x {item.name}</strong>{item.size && <span> · {item.size.name}</span>}{item.edge && <span> · Borda {item.edge.name}</span>}{item.additionals?.length > 0 && <span> · {item.additionals.map((additional) => additional.name).join(", ")}</span>}{item.note && <small> · {item.note}</small>}<br /><Price value={item.subtotal || item.unit_price * item.quantity} /></li>)}</ul><p><strong>Total: <Price value={order.total} /></strong></p></section></main>;
}
