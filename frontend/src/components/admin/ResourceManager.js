...
export function ResourceManager({ title, api, fields, priceField, imageField }) {
  ...
  async function remove(item) {
    ...
    try { await api.remove(item.id); setMessage(""); notify("Item excluído com sucesso."); await load(); }
    catch (error) { setMessage(error.message); }
  }
    ...
    <div className={styles.resourceLayout}>
      ...
      <section className={styles.panel}>
        ...
        <div className={`${styles.panelBody} ${styles.resourceListBody}`}>
          <div className={styles.resourceSearch}><label className={styles.filterControl}><span>Buscar</span><Search size={18} /><input aria-label={`Buscar ${title.toLowerCase()}`} placeholder={`Buscar ${singular}…`} value={query} onChange={(event) => setQuery(event.target.value)} /></label></div>
          {loading ? <AdminLoader fullScreen label="Carregando itens..." /> : filtered.length === 0 ? <p className={styles.empty}>Nenhum item encontrado.</p> :
            <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Nome</th>{priceField && <th>Preço</th>}<th>Ações</th></tr></thead>
              <tbody>{filtered.map((item) => {
                ...
                return <tr key={item.id}><td data-label="Nome"><div className={styles.productCell}>{imageField && <span className={styles.resourceImage} style={{ backgroundImage: `url("${imageUrl}")` }} />}<strong className={imageField ? styles.resourceName : undefined}>{item.name}</strong></div></td>{priceField && <td data-label="Preço">{formatCurrency(item[priceField])}</td>}<td className={styles.resourceActionsCell} data-label="Ações"><div className={styles.actions}>
                <button className={styles.iconButton} onClick={() => startEditing(item)} aria-label={`Editar ${item.name}`}><Pencil size={17} /></button>
                <button className={`${styles.iconButton} ${styles.danger}`} onClick={() => remove(item)} aria-label={`Excluir ${item.name}`}><Trash2 size={17} /></button>
              </div></td></tr>;
              })}</tbody></table></div>}
        </div>
      </section>
