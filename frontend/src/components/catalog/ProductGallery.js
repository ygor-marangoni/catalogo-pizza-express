...
export function ProductGallery({ product }) {
  return <div className={styles.gallery}>
    <div className={styles.mainImage}>{product.images[0] ? <Image src={product.images[0]} alt={`Imagem principal de ${product.name}`} fill loading="eager" sizes="(max-width: 780px) 100vw, 55vw" /> : <span>{product.name.slice(0, 1)}</span>}</div>
