...
export class ApiCatalogRepository {
  ...
  async getFeaturedProducts() { return (await this.loadCatalog()).products.filter((item) => item.featured); }
  async getPopularProducts() { return (await this.loadCatalog()).products.filter((item) => item.popular); }
  ...
  async getProductBySlug(slug) { return (await this.loadCatalog()).products.find((item) => item.slug === slug) || null; }
  async searchProducts(term) {
    try {
      ...
      return products.filter((product) => productMatchesSearch(product, term, categories));
    }
  }
}
