...
const controller = createCrudController(services.size, "Sizes");
...
const sizeSchema = z
	...
	.strict();
router.post("/", protect, adminOnly, validate(sizeSchema), controller.create);
router.put("/:id", protect, adminOnly, validate(sizeSchema.partial()), controller.update);
