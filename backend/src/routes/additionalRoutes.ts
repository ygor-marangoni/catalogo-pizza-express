...
const controller = createCrudController(services.additional, "Additionals");
...
const additionalSchema = z
	...
	.strict();
router.post("/", protect, adminOnly, validate(additionalSchema), controller.create);
router.put("/:id", protect, adminOnly, validate(additionalSchema.partial()), controller.update);
